...
<CodeBlock className="mt-6" language="bash" code={`npx @venator-ui/cli init ai-tool`} />
...
<CodeBlock
  ...
  tailwind.config.js     # Pre-configured with venatorUIPreset
...
- **`@venator-ui/tokens`** — included via `venatorUIPreset` in `tailwind.config.js`
...
<CodeBlock className="mt-6" language="bash" code={`npx @venator-ui/cli add page-header`} />
